...
def need_to_update_existing_file(stats_dir, filestring, frequency, dstart, dend,climatology_start):    
    ...
    if not update_existing_file:
        ...
        resolution = "D" if frequency == "daily_mean" else "MS"
...
# filename, so "2026-08-01" and "20260801" must not behave differently: the
...
# One entry per monitored quantity: the product file that holds it, the name of
# the variable inside that file, and its units. Unlike SRB and ERB, a single
# CLARA-A3 cloud file carries several distinct geophysical variables -- the
# cloud top file holds temperature, height and pressure, and the liquid and ice
# files each hold a water path, an optical thickness and an effective radius --
# so the monitored quantity, the file it comes from and the netCDF variable
# name are three different things. The key is what appears in the aux filenames
# and in the gallery, and the units differ from one quantity to the next, which
# they never did for a radiation budget.
MONITORED = {
...
# The joint cloud property histogram (JCH) is deliberately absent: it is a 6-D
# field (phase x CTP bin x COT bin x lat x lon) on a 1x1 degree grid, so the
...
# the CDR and I1GL the ICDR. The tail varies with the platform code as well, so
# the files are looked up rather than constructed: one directory listing per
# month, cached, instead of a hard-coded name that silently reports a file as
# missing when the tail changes.
FILE_PATTERN = re.compile(r"(?P<prefix>[A-Z]{3})(?P<token>dm|mm|mh)(?P<date>\d{8})")


@lru_cache(maxsize=None)
def month_index(freq, year, month):
    """{(prefix, YYYYMMDD): path} for one <year>/<month> folder of the dataset."""
    directory = dir_dataset / freq / f"{year}" / f"{month:02d}"
    index = {}
    if directory.is_dir():
        for path in sorted(directory.glob("*.nc")):
            match = FILE_PATTERN.match(path.name)
            if match:
                index[(match.group("prefix"), match.group("date"))] = str(path)
    return index


def dataset_file(date, freq, prefix=None):
    """Path of the file holding one date at one frequency, or None if absent.

    This is the only place that knows how the dataset lays out its files, so both
    the main file list and the climatology block below go through it.
    """
    return month_index(freq, date.year, date.month).get(
        (prefix or file_prefix, date.strftime("%Y%m%d")))


dates = pd.date_range(dstart, dend, freq="D" if frequency.startswith("daily") else "MS")

paths = {date: dataset_file(date, frequency) for date in dates}
existing_fnames = [path for path in paths.values() if path is not None]
missing_dates = [date for date, path in paths.items() if path is None]
# Only the files that are actually there: a date missing from the delivery would
# otherwise abort the whole run when open_mfdataset reached it.
fnames = existing_fnames

print(f"Expected: {len(dates)}")
print(f"Existing: {len(existing_fnames)}")
print(f"Missing:  {len(missing_dates)}")

for date in missing_dates:
    print(f"No {file_prefix} {frequency} file for {date:%Y-%m-%d}")
...
if do_compute_stats_tseries:
    ...
    # Every task was a file that existed when the run started, so anything
    # unreadable here disappeared while the run was in progress (a truncated time
    # series would otherwise be written without any visible error).
    if absent:
