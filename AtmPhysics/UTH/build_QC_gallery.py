...
ECV = "UTH"
PRODUCT = "UTH"
DATASET_TITLE = "Upper Tropospheric Humidity"
UNITS = "%"

DATADIR = Path(f"../../../datasets/{ECV}")
...
# Unlike the other datasets there is no product level below the ECV: UTH is a
# single product, so the site path stops at the ECV, mirroring the layout of
# this directory in the repository.
UPLOAD_TARGET = f"dataset_qc/{DOMAIN}/{ECV}/"
UPLOAD_SPACE = "cxjo"
UPLOAD_SITE = "ecv-info"

# Edition 2 is delivered as daily means only, so the frequency tabs carry a
# single entry. The per-frequency machinery is kept as it stands in the other
# galleries: it costs nothing here and a monthly edition would slot straight in.
FREQUENCIES = ("daily",)
FREQ_LABELS = {"daily": "Daily"}
FREQ_RESOLUTION = {"daily": "D"}

# One product variable at one frequency. UTH is the key used in the aux
# filenames and in the gallery; mean_uth is the field inside the files.
PRODUCTS = {
    "daily": ["UTH"],
}
PRODUCT_PREFIX = {"UTH": "UTH"}
MONITORED_FIELD = {"UTH": "mean_uth"}

# Fields to map, grouped by the product file that holds them. Two fields the
# files carry are deliberately absent: hours_contributing_to_daily_mean and
# satellites_contributing_to_daily_mean are bit masks (one bit per UTC hour and
# per platform), so a continuous colour scale over them would be meaningless.
# The sampling density they describe is already covered by
# number_of_observations.
MAP_FIELDS = {
    "daily": {
        "UTH": ["mean_uth", "min_uth", "max_uth",
                "number_of_observations", "number_of_missing_observations",
                "sampling_uncertainty", "total_uncertainty", "mean_rh_delta"],
    },
}

# Map gallery covers the last N months of the dataset, one map per month.
# For daily data the map is taken from GALLERY_DAY of each month.
N_GALLERY_MONTHS = 6
GALLERY_DAY = 1

# First day of the record. The delivery starts part-way through July 1994, so
# this is the first file date rather than the first of the month: starting the
# expected-date range any earlier would report days as missing that the dataset
# never claimed to provide.
COLLECTION_START = pd.Timestamp("1994-07-06")
# None -> auto-detect from the newest file found for each (frequency, variable).
COLLECTION_END = None

# Filename pattern: UTH<dm><YYYYMMDD>000000220MSPOS<01|I1>GL.nc
...
# white, and no colour scale is allowed anywhere near white, so a zero or a
# minimum of the range can never be mistaken for a gap in the retrieval.
# Sequential palettes that fade to white at one end (the ColorBrewer ones, and
# the pale ends of magma and inferno) are trimmed to enforce that; a palette
# that passes *through* white in the middle, which is how almost every diverging
# palette is built, cannot be trimmed and has to be swapped for one that is dark
# in the middle instead (Crameri's "berlin" here).
NAN_COLOR = "white"
...
# valid_min drops a sentinel that survives decoding. The two count fields
# declare _FillValue = -999 but actually store the int32 minimum in their
# missing cells, so xarray masks nothing and the field arrives holding
# -2147483648. Left in, it takes the whole colour scale with it.
MAP_STYLES = {
    "mean_uth": dict(cmap="YlGnBu", levels=np.arange(0, 101, 10)),
    "min_uth": dict(cmap="YlGnBu", levels=np.arange(0, 101, 10)),
    "max_uth": dict(cmap="YlGnBu", levels=np.arange(0, 101, 10)),
    "number_of_observations": dict(cmap="Greens", levels=None, valid_min=0),
    "number_of_missing_observations": dict(cmap="Oranges", levels=None, valid_min=0),
    "sampling_uncertainty": dict(cmap="magma_r", levels=None),
    "total_uncertainty": dict(cmap="cividis", levels=None),
    # The RH departure diverges about zero; "berlin" is RdBu_r without the
    # white midpoint, which here would be indistinguishable from a NaN.
    "mean_rh_delta": dict(cmap="berlin", levels=None),
}

# Units of the mapped fields, for the map titles. The uncertainties that come
# from the brightness temperatures are in kelvin, not in the percent the
# humidity itself is reported in.
FIELD_UNITS = {
    "mean_uth": "%",
    "min_uth": "%",
    "max_uth": "%",
    "number_of_observations": "1",
    "number_of_missing_observations": "1",
...
def create_qc_figure(stats_path, title, units, thresholds_path=None, variable=None):
    ...
    # compute_stats.py for this dataset does not derive climatological P0.1/P99.9
    # thresholds, so the outlier fractions the other galleries draw in the last
    # panel are absent. The panel still carries data completeness; the traces and
    # the panel title follow what the statistics file actually holds, so this
    # gallery needs no change once the thresholds are added.
    has_outliers = {"negative_outliers_fraction",
                    "positive_outliers_fraction"} <= set(stats.columns)
    ...
    fig = make_subplots(
        ...
        subplot_titles=[
            ...
            "Data completeness and climatological outliers" if has_outliers
            else "Data completeness",
        ],
    )
    ...
    # The hover carries the valid-pixel count as well as the missing one. This
    # dataset's statistics file records both, and on a fixed grid the pair says
    # directly how much of the globe the daily mean actually rests on.
    hover_columns = ["missing"]
    hover_lines = ["Missing fraction: %{y:.4%}", "Missing pixels: %{customdata[0]:.0f}"]
    if "number_of_values" in stats.columns:
        hover_columns.append("number_of_values")
        hover_lines.append("Valid pixels: %{customdata[1]:.0f}")
    fig.add_trace(
        go.Scatter(
            x=stats["time"], y=stats["missing_fraction"], mode="lines", name="Missing fraction",
            customdata=stats[hover_columns].values,
...
def save_map(fname, field, output_path, title):
    ...
    # See the note on MAP_STYLES: the count fields carry an undeclared int32
    # sentinel in their missing cells, which has to go before the levels are
    # derived from the data.
    valid_min = style.get("valid_min")
    ...
    kwargs = {
        ...
        # Trimmed of white and with NaN set to white, so that the blank areas
        # of a map are unambiguously missing data rather than a low value.
...
def save_spatial_map(nc_path, statistic, output_path, title):
    ...
    style = ekp.styles.Style(
        # Same rule as the per-date maps: the colours are trimmed of white,
        # and earthkit-plots leaves NaN transparent over the white figure.
        ...
        for variable, fields in MAP_FIELDS[f].items():
            ...
            for field in fields:
                ...
                for i, date in enumerate(dates, 1):
                    ...
                    title = (f"{PRODUCT} {field}"
                             + (f" [{units}]" if units else "")
                             + f" — {FREQ_LABELS[f]} — {label}")
