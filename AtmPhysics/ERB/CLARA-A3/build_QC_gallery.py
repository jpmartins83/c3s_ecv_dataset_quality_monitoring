...
ECV = "ERB"
PRODUCT = "CLARA-A3"
DATASET_TITLE = "CLARA-A3 Earth Radiation Budget"
UNITS = "W m-2"
...
# One NetCDF file per product variable, both delivered at both frequencies.
PRODUCTS = {
    "monthly_mean": ["OLR", "RSF"],
    "daily_mean": ["OLR", "RSF"],
}

# Fields to map, grouped by the product file that holds them. Note that for ERB
# the geophysical variable inside the file is not named after the file itself:
# the OLR files hold LW_flux and the RSF files hold SW_flux.
MAP_FIELDS = {
    "monthly_mean": {
        "OLR": ["LW_flux", "number_of_lw_inst_obs", "number_of_lw_daily_means"],
        "RSF": ["SW_flux", "SW_flux_twilight", "number_of_sw_inst_obs",
                "number_of_sw_daily_means", "relative_share_twilight",
                "relative_share_daylight", "relative_share_sunglint"],
    },
    "daily_mean": {
        "OLR": ["LW_flux", "number_of_lw_inst_obs"],
        "RSF": ["SW_flux", "SW_flux_twilight", "number_of_sw_inst_obs",
                "relative_share_twilight", "relative_share_daylight",
                "relative_share_sunglint"],
    },
}
...
MAP_STYLES = {
    ...
    "LW_flux": dict(cmap="inferno", levels=np.arange(60, 381, 20)),
    "SW_flux": dict(cmap="inferno", levels=np.arange(0, 421, 20)),
    "SW_flux_twilight": dict(cmap="magma", levels=np.arange(0, 33, 3)),
    # Observation counts — the range differs between the two frequencies, so the
    # levels are derived from the data itself.
    "number_of_lw_inst_obs": dict(cmap="Greens", levels=None),
    "number_of_sw_inst_obs": dict(cmap="Greens", levels=None),
    "number_of_lw_daily_means": dict(cmap="Blues", levels=np.arange(0, 32, 2)),
    "number_of_sw_daily_means": dict(cmap="Blues", levels=np.arange(0, 32, 2)),
    # Relative temporal shares [%]
    "relative_share_twilight": dict(cmap="PuBuGn", levels=np.arange(0, 101, 10)),
    "relative_share_daylight": dict(cmap="YlOrBr", levels=np.arange(0, 101, 10)),
    "relative_share_sunglint": dict(cmap="BuPu", levels=np.arange(0, 101, 10)),
}
