...
def compute_file_stats(task):
    ...
    if v.size == 0:
        # Completely empty field (no observations at all that day — there are 66
        # such days in the ERB daily record). The xarray reductions returned NaN
        # for every statistic here, so reproduce that rather than dividing by a
        # zero weight or asking numpy for the percentile of an empty array.
        return {
            "time": timestamp,
            "mean": np.nan,
            "median": np.nan,
...
file_prefix = var_string   # for CLARA the filename prefix is the variable token
nc_var = nc_variables[var_string]
freq_string = freq_strings[frequency]
    
cdr_end = pd.Timestamp("2020-12-31")  # last date of the CDR; later dates are ICDR files
...
def dataset_file(date, freq):
    ...
    """
    suffix = "01GL.nc" if date <= cdr_end else "I1GL.nc"
    return (f"../../../datasets/{ECV}/{PRODUCT}/{freq}/{date.year}/{date.month:02d}/"
            f"{file_prefix}{freq_strings[freq]}{date:%Y%m%d}000000319AVPOS{suffix}")
