...
ECV = "Precipitation"
PRODUCT = "GIRAFE"
DATASET_TITLE = "GIRAFE Precipitation"
UNITS = "mm"

DATADIR = Path(f"../../../datasets/{PRODUCT}")
...
FREQ_LABELS = {"monthly": "Monthly", "daily": "Daily"}
FREQ_RESOLUTION = {"monthly": "MS", "daily": "D"}

# One product variable, delivered at both frequencies. The name is the one used
# in the aux filenames and inside the files; PRODUCT_PREFIX is what the dataset
# filenames use instead.
PRODUCTS = {
    "monthly": ["precipitation"],
    "daily": ["precipitation"],
}
PRODUCT_PREFIX = {"precipitation": "PRE"}

# Fields to map, grouped by the product file that holds them. The two
# frequencies genuinely carry different fields, so they are listed separately:
# num_days/quality_flag/num_days_snow exist only in the monthly files, and
# sampling_error/snow_flag only in the daily ones.
MAP_FIELDS = {
    "monthly": {
        "precipitation": ["precipitation", "num_obs_fraction", "num_obs_rate",
                          "num_days", "quality_flag", "num_days_snow"],
    },
    "daily": {
        "precipitation": ["precipitation", "num_obs_fraction", "num_obs_rate",
                          "sampling_error", "sampling_error_quality_flag", "snow_flag"],
    },
}
...
# Filename pattern: PRE<dm|mm><YYYYMMDD>000000120IMPGS<01|I1>GL.nc
...
MAP_STYLES = {
    ...
    "sampling_error": dict(cmap="cividis", levels=None),
    "sampling_error_quality_flag": dict(cmap="tab10", levels=None),
    "snow_flag": dict(cmap="cool", levels=None),
}
